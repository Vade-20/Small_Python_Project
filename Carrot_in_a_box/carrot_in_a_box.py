...
def main(stdsrc):
    ...
    '''
    add_wrapped_str(stdsrc,0,5,rules,width)
    stdsrc.addstr(10,0,'Player 1 name : ')
    player_1 = stdsrc.getstr(10,16).decode(encoding="utf-8")
    stdsrc.addstr(12,0,'Player 2 name : ')
    player_2 = stdsrc.getstr(12,16).decode(encoding="utf-8")
    stdsrc.addstr(14,0,'HERE ARE THE TWO BOXES--->')
    box_1 = curses.newwin(15,20,21,0)
    box_1.addstr(boxes['red_box_close'])
    box_1.addstr(f'\n{player_1}')
    box_1.refresh()
    box_2 = curses.newwin(11,20,35,0)
    box_2.addstr(boxes['gold_box_close'])
    box_2.addstr(f'\n{player_2}')
    box_2.refresh()
    
    rule_box = curses.newwin(13,80,23,35)
