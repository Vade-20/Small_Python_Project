...
def back():
    ...
    for i in num_of_voices:
        if i[0] == option_var.get():
            engine.setProperty('voice', i[1])
    rate = int(spin_3.get())
    engine.setProperty('rate',rate)
    volume = int(spin_4.get())/100
...
def setting():
    global option_2,spin_3,spin_4,option_var,rate,volume
    ...
    spin_3 = Spinbox(root, from_= 0, to = 4000,increment=10,validate="key",textvariable=spin_3, validatecommand=vcmd,justify='center',font=('Times New Roman',15),fg=FONT_COLOR, bg=BACKGROUND_COLOR ) 
    spin_3.grid(row=3,column=1,sticky=W+E)

    
    spin_4 = Spinbox(root, from_= 0, to = 100,increment=10,validate="key",textvariable=spin_4, validatecommand=vcmd,justify='center',font=('Times New Roman',15),fg=FONT_COLOR, bg=BACKGROUND_COLOR ) 
    spin_4.grid(row=4,column=1,sticky=W+E)
    spin_4.delete(0,'end')
    spin_4.insert(0,int(volume*100))
...
def main():
    ...
    t1 = scrolledtext.ScrolledText(root, wrap = WORD, width = 40, height = 10, font = ("Times New Roman",15),fg=FONT_COLOR) 
